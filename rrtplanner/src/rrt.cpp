...
Rrt::Rrt(const ros::NodeHandle& nh)
  ...
  , pmap_sub_(nh_.subscribe("pmap", 1, &Rrt::pmapCallback, this))
  ...
  , distr(0, 1)
{
  ...
  if (!ros::param::get(ns + "/tree/extension_range", extension_range_))
...
void Rrt::execute(const rrtplanner::rrtGoalConstPtr& goal)
{
  ...
  kdtree* kd_tree = kd_create(3);    // Dimension이 3인 root tree, goal tree를 만듦
  kdtree* goal_tree = kd_create(3);  // kdtree는 root,rect(범위), 함수포인터를 가짐
  for (int i = 0; i < goal->goal_poses.poses.size(); ++i)
  {
    ...
    // goal tree의 root로 goal을 등록하고, rect로 goal의 pose를 등록한다.(min,max)
    // kdnode : *pos, dir, *data, *left, *right를 가지며
  ...
  // kd_tree의 root로 start point를 등록하고, rect로 start point를 등록한다.
...
void Rrt::pmapCallback(const nav_msgs::OccupancyGrid &pmap)
{
  pmap_ = std::make_shared<nav_msgs::OccupancyGrid>(pmap);
  //std::cout << pmap_ << std::endl;
}
...
Eigen::Vector3d Rrt::sample(geometry_msgs::PoseStamped start)
{
  ...
  for (int i = 0; i < 3; ++i)
  {
    do
    {
        x_samp[i] = (boundary_max_[i] - boundary_min_[i])
            * distr(eng) + boundary_min_[i];
    } while(x_samp[i] < boundary_min_[i] or x_samp[i] > boundary_max_[i]);
  }

  x_samp[0] += start.pose.position.x;
  x_samp[1] += start.pose.position.y;
  x_samp[2] += start.pose.position.z;
  */

  int xmin = pmap_->info.origin.position.x;
  int ymin = pmap_->info.origin.position.y;
  int xmax = pmap_->info.width * pmap_->info.resolution
      + pmap_->info.origin.position.x;
  int ymax = pmap_->info.height * pmap_->info.resolution
      + pmap_->info.origin.position.y;

  int xidx, yidx;

  do
  {
    x_samp[0] = (xmax - xmin) * distr(eng) + xmin;
    x_samp[1] = (ymax - ymin) * distr(eng) + ymin;
//    x_samp[2] = (boundary_max_[2] - boundary_min_[2])
//        * distr(eng) + boundary_min_[2] + start.pose.position.z;

    xidx = (int) (x_samp[0] - xmin / pmap_->info.resolution);
    yidx = (int) (x_samp[1] - ymin / pmap_->info.resolution);

  } while (!( pmap_->data[ yidx * (pmap_->info.width) + xidx] >= 0 ));
...
{
  // kdres : tree / res_node(kdnode, distance, next resnode 를 갖는) 2개(rlist, riter)
  // riter은 rlist의 다음 resnode를 가리킴 (여기서는 0)
  //
...
bool Rrt::collisionLine(Eigen::Vector3d p1, Eigen::Vector3d p2, double r)
{
  ...
  octomap::point3d dir_floor(0,0,-1);
...
//  std::cout << p2_floor << std::endl;
//  octomap::point3d query(p2[0], p2[1], p2_floor.z());
...
void Rrt::visualizeEdge(RrtNode* node, int id)
{
  ...
  a.color.b = 0.7;
  a.color.a = 1.0;
  a.lifetime = ros::Duration(60.0);
  a.frame_locked = false;

  path_pub_.publish(a);
}

void Rrt::visualizeSamp(Eigen::Vector3d node, int id)
{
  visualization_msgs::Marker a;
  a.header.stamp = ros::Time::now();
  a.header.seq = id;
  ...
  a.type = visualization_msgs::Marker::SPHERE;
  a.action = visualization_msgs::Marker::ADD;
  a.pose.position.x = node[0];
  a.pose.position.y = node[1];
  a.pose.position.z = node[2];
  a.scale.x = 1.0;
  a.scale.y = 1.0;
  a.scale.z = 1.0;
  a.color.r = 0.3;
...
void Rrt::visualizeNew(Eigen::Vector3d node, int id)
{
  visualization_msgs::Marker a;
  a.header.stamp = ros::Time::now();
  a.header.seq = id;
...
void Rrt::visualizeParent(RrtNode* node, int id)
{
  visualization_msgs::Marker a;
  a.header.stamp = ros::Time::now();
  a.header.seq = id;
  ...
  a.type = visualization_msgs::Marker::SPHERE;
  a.action = visualization_msgs::Marker::ADD;
  a.pose.position.x = node->pos[0];
  a.pose.position.y = node->pos[1];
  a.pose.position.z = node->pos[2];
  a.scale.x = 0.2;
  a.scale.y = 0.2;
  a.scale.z = 0.2;
  a.color.r = 0.0;
