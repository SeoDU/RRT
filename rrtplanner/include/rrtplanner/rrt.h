...
public:
  ...
  void pmapCallback(const nav_msgs::OccupancyGrid &pmap);
  ...
  void visualizeParent(RrtNode* node, int id = 0);
  void visualizePath(RrtNode *node);
  void visualizeSamp(Eigen::Vector3d node, int id=0);
  void visualizeNew(Eigen::Vector3d node, int id=0);

  Eigen::Vector3d sample(geometry_msgs::PoseStamped start);
...
private:
  ...
  std::shared_ptr<nav_msgs::OccupancyGrid> pmap_;

  ros::Subscriber octomap_sub_;
  ros::Subscriber pmap_sub_;
